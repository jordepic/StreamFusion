...
| Filter (`WHERE`) | Conjunctions of comparisons | A pure filter (identity projection) whose predicate is a comparison — or an `AND` of comparisons — of a numeric column against a numeric literal (`>` `>=` `<` `<=` `<>`, either order) or a string column against a string literal (`<>`); `BETWEEN`/ranges expand to comparisons and route. The whole row is carried through Arrow, so every input column must be a type the [whole-row converter](src/main/java/io/github/jordepic/streamfusion/operator/RowDataArrowConverter.java) handles (the primitives, boolean, string). Equality (`=`) on the filtered column falls back (the column is constant-folded into a projection — pending projection support); disjunctions, non-comparison terms, projections, and other column types fall back. |
