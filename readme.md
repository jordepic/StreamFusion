...
- Kafka value formats on the native decode path beyond CSV/raw/bare-Avro: the registry-framed `avro-confluent` and `protobuf` decoders exist but aren't planner-wired yet; a `key.format`, a topic pattern, an unsupported startup/bounded mode, or untranslatable consumer properties also fall back
