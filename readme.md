...
- Expressions using operations the engine does not admit: scalar functions outside the admitted set (e.g. `CONCAT`, whose NULL semantics differ — [divergences/07](divergences/07-expression-encoding-and-compile-once.md) — and most math/temporal functions), and narrowing/float→int/string `CAST`
