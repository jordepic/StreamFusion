...
| Tumbling window aggregate | Yes | Event-time `TUMBLE` over a local-time-zone (rowtime) attribute; one or more aggregates over the same value column — `SUM` / `MIN` / `MAX` / `COUNT` (and `AVG` only as a lone aggregate); grouped by the window, optionally plus one or more bigint/int/string keys. Value-type support is the parity intersection in [docs/aggregate-type-support.md](docs/aggregate-type-support.md): all five over bigint/double, `SUM`/`MIN`/`MAX`/`COUNT` over int (`SUM` keeps the host's wrapping int semantics, `AVG` its integer-truncating semantics), and `MIN`/`MAX`/`COUNT` over smallint/tinyint/float (the type-preserving comparisons/counts). `AVG` applies only to integer values (bigint/int), never double. |
...
- `SUM`/`AVG` over smallint/tinyint/float, and any aggregate over decimal — see [docs/aggregate-type-support.md](docs/aggregate-type-support.md)
