...
| Streaming Top-N (`ROW_NUMBER`) | Yes (emits a changelog) | An append-only streaming Top-N — `ROW_NUMBER() OVER (PARTITION BY … ORDER BY …) <= N`, **with or without the rank number projected**. Keeps the best `N` rows per partition (sorted by the order keys, ties in arrival order) and emits a changelog as that set changes — matching the host's append-only bounded buffer, the closest analog being RisingWave's `TopNCache` ([divergences/14](divergences/14-standalone-streaming-engines.md)) though we hold the full set in memory. **Without the rank number**: an `INSERT` for a row entering the top-N and a `DELETE` for one displaced (delete before insert, and nothing when the new row is itself rank N+1). **With the rank number** (`SELECT … rn …`): a row entering at rank `r` shifts everyone below it down by one, so the operator emits the cascade Flink's `AppendOnlyTopNFunction` does — for each rank from `r` to the buffer end an `UPDATE_BEFORE`(old occupant)/`UPDATE_AFTER`(new occupant), and an `INSERT` for a brand-new rank; a row pushed past `N` is retracted by the `UPDATE_BEFORE` at the last rank. The order comparator honors per-column ascending/descending and nulls-first independently. `ROW_NUMBER` only (not `RANK`/`DENSE_RANK`), rank range starting at 1 (no offset), insert-only input; an offset or a retracting input falls back. Columnar (Arrow in/out); the bounded per-partition buffers are checkpointed. |
...
| Keep-first deduplication | Yes | Append-only deduplication — `ROW_NUMBER() OVER (PARTITION BY k ORDER BY rowtime ASC) = 1`, which the host plans as an insert-only row-time deduplicate. Per partition key the operator keeps the **minimum-rowtime** row and emits it once the watermark reaches that rowtime (a faithful port of Flink's `RowTimeDeduplicateKeepFirstRowFunction`: a candidate is replaced only by a strictly smaller rowtime, and emitted on the timer); every later row for the key is then ignored, and a row arriving below the watermark is dropped as late. Insert-only. Keys carried by the columnar shuffle; per-key state checkpointed. Keep-last (`ORDER BY rowtime DESC`) is a retracting deduplicate and falls back. Columnar (Arrow in/out). |
...
- Streaming Top-N over a *retracting* input — only the append-only Top-N is implemented (it consumes insert-only rows). The non-windowed `GROUP BY` aggregate and the regular (updating) join both consume and emit a changelog natively; the non-windowed `GROUP BY` does not model `AVG`'s multi-field running state directly (it works via the host's `SUM`/`COUNT` rewrite).
