...
| Kafka source native decode (`SELECT … FROM`) | CSV / raw value formats | A `kafka`-connector source whose `value.format` is `csv` or `raw`: Flink's own Kafka source consumes the raw value bytes — owning offsets, checkpointing, and auth — and a native operator decodes a whole batch straight to Arrow, replacing Flink's per-record `RowData` materialization with one native decode per batch. Requires an explicit topic, a supported startup mode, an unbounded or `latest-offset` bounded scan, and no `key.format`. (A JSON value table takes the native Kafka *source* instead where that is eligible.) Avro and protobuf have native decoders but are not wired into this routing yet, so they fall back. |
...
- Kafka value formats on the native decode path beyond CSV/raw: Avro and protobuf decoders exist but aren't planner-wired yet; a `key.format`, a topic pattern, an unsupported startup/bounded mode, or untranslatable consumer properties also fall back
