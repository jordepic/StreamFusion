...
- **Insert-only input**, with one exception: the non-windowed `GROUP BY` aggregate both emits and consumes a changelog (see its row above). Every other operator consumes insert-only input; a retracting or updating *input* to them falls back to Flink.
