...
class StatefulTumblingTest {
  ...
  void accumulatesWindowsAcrossBatchesAndEmitsOnWatermark() {
    try (BufferAllocator allocator = new RootAllocator();
      ...
      long handle = Native.createTumblingAggregator(1000, 1000, 0, new int[] {0}); // SUM
      try {
  ...
  void restoresMultiFieldAvgStateFromSnapshot() {
    try (BufferAllocator allocator = new RootAllocator();
      ...
      long handle = Native.createTumblingAggregator(1000, 1000, 0, new int[] {4});
      byte[] snapshot;
      try {
      ...
      long restored = Native.restoreTumblingAggregator(1000, 1000, 0, new int[] {4}, snapshot);
      try {
  ...
  void twoPhaseMatchesSinglePhase() {
    try (BufferAllocator allocator = new RootAllocator();
      ...
      long local = Native.createTumblingAggregator(1000, 1000, 0, new int[] {0}); // SUM
      long global = Native.createTumblingAggregator(1000, 1000, 0, new int[] {0});
      try {
