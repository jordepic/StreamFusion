...
- **Native Fluss log-table source — done** (was ticket 36). Fluss (columnar streaming storage) is the
  ...
  Method + full numbers in `docs/benchmarks.md` (2026-07-06 watermarked run: 22 of 23 queries
  measured — the windowed family included, q11 peaking at 4.17x; q4/q9/q21 measure to a poison
  marker's output row instead of a row count; only q12 skips, a proctime window being flushable
  only by end-of-input). Watermarked tables route natively (the
