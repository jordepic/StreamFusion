...
- **Native Fluss log-table source — done** (was ticket 36). Fluss (columnar streaming storage) is the
  ...
  Method + full numbers in `docs/benchmarks.md` (2026-07-06 watermarked run over the counting
  blackhole sink: 22 of 23 queries measured, 20 clear 1x with a 0.98x floor, q11 peaking at
  4.02x; q4/q9/q21 measure to a poison marker's output row instead of a row count; only q12
  skips, a proctime window being flushable only by end-of-input). Watermarked tables route natively (the
  assigner runs as the native columnar assigner above the source; a pushed-down spec, should
  Flink ever produce one for Fluss, is regenerated by the shared Kafka machinery or declined by
  shape). The rung's first unbounded runs also flushed out two masked bugs — the missing
  sub-plan-reuse barrier on the Fluss scan and the window re-aggregation shift-zone skew — both
  fixed and pinned by harness tests (see `docs/benchmarks.md`). The partition-removal plumbing
  now matches Fluss's own shape (the split reader answers the partition-to-splits question;
  was ticket 53); ticket 54 holds the upstream `scan.bounded.mode` contribution that would
  collapse the rung's count/sentinel/marker machinery into the Kafka rung's bounded method
  (and admit q12).
...
5. **Operator-level perf** (ticket 20 backlog): the last scalar-keyed maps — the window Top-N
   ...
   formatting (StrftimeItems re-parsed per call) — though the biggest mover was the rung's own
   sink method (the counting blackhole re-quote: q15/q16 now at the line, everything else past
   it). Ranked levers + acceptance benchmarks in the research note.
