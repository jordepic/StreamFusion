...
- **Native Fluss log-table source — done** (was ticket 36). Fluss (columnar streaming storage) is the
  ...
  fixed and pinned by harness tests (see `docs/benchmarks.md`). Ticket 53 now holds only the
  partition-removal-plumbing alignment cleanup; ticket 54 holds the upstream
  `scan.bounded.mode` contribution that would collapse the rung's count/sentinel/marker
  machinery into the Kafka rung's bounded method (and admit q12).
