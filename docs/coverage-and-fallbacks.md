...
|---|---|
| `Correlate` | lateral table functions, and `UNNEST` beyond the INNER `ARRAY`/`MAP` case (`MULTISET` unnest, `WITH ORDINALITY`, `LEFT` unnest, or a pushed condition the expression engine can't encode). INNER `UNNEST` of a single `ARRAY` (scalar or `ROW` element, flattened) or `MAP` (key+value) column — including a pushed element filter — **is** supported (see the chart). |
