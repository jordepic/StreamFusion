...
|---|---|
| `Correlate` | lateral table functions, and `UNNEST` beyond the INNER single-`ARRAY` case (`MAP`/`MULTISET` unnest, `WITH ORDINALITY`, `LEFT` unnest, or a pushed condition the expression engine can't encode). INNER `UNNEST` of a single `ARRAY` column — scalar or `ROW` element (flattened), including a pushed element filter — **is** supported (see the chart). |
